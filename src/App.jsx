...
function renderGraph(root, data) {
  const layout = {
    title: `Commits to the torvalds/linux repo (${new Date(data[0].week * 1000).toLocaleDateString()} - ${new Date(data[data.length - 1].week * 1000).toLocaleDateString()})`,
    xaxis: {
      title: "Week",
      showgrid: true,
      zeroline: false,
...
function renderGrid(root, data) {
  const layout = {
    title: `Commits to the torvalds/linux repo, by day`,
    height: 350,
  };
...
function Example() {
  const { isPending, error, data } = useQuery({
    ...
    queryFn: () =>
      fetch(
        "https://api.github.com/repos/torvalds/linux/stats/commit_activity",
      ).then((res) => res.json()),
  });
  ...
  return (
    ...
    <h1 className={styles.title}>Linux kernel repo commit activity</h1>
      <div
        ref={(root) => {
          if (!root) return;
